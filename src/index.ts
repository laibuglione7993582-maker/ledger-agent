 ...
 * ledger-agent
 *
 * LedgerAgent and governance wrapper for x402 payments.
...
export class BudgetExceededError extends Error {
  constructor(
    ...
    super(
      `ledger-agent: ${kind} budget would be exceeded ` +
 ...
 *   onRecord: (r) => console.log("[governance]", r),
 * });
 *
 * const { response, record } = await agent.fetch("https://api.example.com/data");
 * ```
 */
export function createLedgerAgent(options: LedgerAgentOptions) {
  const fetchImpl = options.fetch ?? globalThis.fetch;
  if (!fetchImpl) {
    throw new Error(
      "ledger-agent: no fetch implementation found. Pass options.fetch or run on Node 18+.",
    );
  }
  ...
  async function wrappedFetch(
    ...
    const paid = await fetchImpl(input, {
      ...
      headers: {
        ...(init?.headers ?? {}),
        "x-payment-authorization": `governance/${options.agent}`,
      },
    });
